...
// TODO, make all messages a multiple of CACHE_LINE to make flushing/invalidating safer

status_t mailbox_write(const enum mailbox_channel ch, uint32_t value) {
    value = value | ch;
...
status_t mailbox_read(enum mailbox_channel ch, uint32_t *result) {
...
static status_t mailbox_get_framebuffer(fb_mbox_t *fb_desc_in) {
    status_t ret = NO_ERROR;

    arch_clean_cache_range((addr_t)fb_desc_in,sizeof(fb_mbox_t));

    ret = mailbox_write(ch_framebuffer, kvaddr_to_vc_bus((addr_t)fb_desc_in));
    if (ret != NO_ERROR)
        return ret;
    ...
    arch_invalidate_cache_range((addr_t)fb_desc_in,sizeof(fb_mbox_t));
